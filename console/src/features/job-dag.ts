  ...
  const nodes = operators.map((operator: any, index: number) => {
    ...
    return {
      ...
      data: {
        ...
        key_field: operator.key_field,
      },
    }
  })
  const edges = (Array.isArray(spec.edges) ? spec.edges : []).map((edge: any) => ({
    id: edge.id ?? `${edge.from}-${edge.to}`,
    source: edge.from,
    target: edge.to,
    data: { partitioned: edge.partitioned !== false },
    label: edge.partitioned !== false ? 'partitioned' : undefined,
    animated: false,
  }))
  return { nodes, edges }
}
...
export function dagToJobSpec(nodes: DagNode[], edges: Edge[], base: JobSpec = {}): JobSpec {
  ...
  const operators = nodes.map((node) => {
    ...
    if (data.kind === 'source')
      sources.push({
        operator_id: node.id,
        input_type: data.component,
        config: clone(data.config ?? {}),
        time: clone(
          base.sources?.find((item: any) => item.operator_id === node.id)?.time ?? {
            mode: 'processing_time',
          },
        ),
      })
    if (data.kind === 'sink')
      sinks.push({ operator_id: node.id, output_type: data.component, config: clone(data.config ?? {}) })
    const config = clone(data.config ?? {})
    if (data.kind === 'processor' && !config.type) config.type = data.component
    return {
      id: node.id,
      kind: data.kind === 'processor' ? data.operatorKind || 'map' : data.kind,
      ...(data.stateful ? { stateful: true, key_field: data.key_field || undefined } : {}),
      config,
    }
  })
  return {
    ...
    edges: edges.map((edge) => ({
      ...
      from: edge.source,
      to: edge.target,
      partitioned: (edge.data as any)?.partitioned !== false,
    })),
    sources,
    sinks,
  }
