...
| processor | `milvus_search` | Searches a Milvus collection for the top-k nearest neighbors of each row's vector and appends the matches as a JSON array text column. Rows are searched with bounded, order-preserving concurrency. | [reference](../components/processors/milvus-search) |
