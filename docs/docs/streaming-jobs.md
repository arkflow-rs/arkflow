...
see [Distributed jobs](concepts/7-distributed-jobs.md) for the runtime
contract.
...
Field-by-field reference: [Top-level configuration → job](configuration/1-top-level.md#job).
...
2. **Submit stopped, inspect, then run.** Creation is a desired-state write
   ...
   [HTTP API v1](control-plane/http-api-v1.md)).
