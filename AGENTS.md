...
- Example YAMLs are all registered in `docs/reference/example-manifest.json` and deep-validated by a workspace test (`crates/arkflow/tests/examples_validate.rs`); `docs:check` fails if a YAML under `examples/` is not registered, and an example that cannot validate offline needs explicit `"validate": false` plus a reason in the manifest.
- README.md / README_zh.md component lists live inside `<!-- README_COMPONENTS:<kind> START/END -->` markers, and every bullet states its registry type-name as ``- **Label** (`type_name`): description``; `pnpm docs:check` fails if either README misses, duplicates, or invents components relative to the inventory (see the `documentation-accuracy` spec).
- `pnpm docs:check` also validates internal-link anchors (github-style heading slugs) and sidebar reachability — new pages must appear in `docs/sidebars.ts` or live under an autogenerated directory; legacy stubs are allowlisted in `docs/scripts/docs-check.mjs` until their removal release.
- Every component registers its builder and docs metadata under the same name; `crates/arkflow-plugin/tests/registry_consistency.rs` fails the workspace test run if the two drift apart.
- Every ` ```yaml ` block under `docs/docs/` carries a classification marker in its fence metastring (`validate=full`, `validate=fragment wrap=<kind>`, or `validate=foreign reason="..."`); `pnpm docs:check` rejects unclassified blocks and a workspace test (`crates/arkflow/tests/docs_snippets_validate.rs`) validates them through the real engine parser. See `docs/DOCUMENTATION.md` for the wrap kinds.
...
4. Add an example YAML under `examples/` **and register it in `docs/reference/example-manifest.json`**, add the component docs page with `components:` front matter, and add the type-name bullet to both READMEs' `README_COMPONENTS:<kind>` lists; all of these are CI-checked.
