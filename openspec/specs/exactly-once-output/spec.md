...
The Kafka transactional output (L2) SHALL eliminate in-transaction partial writes and zombie duplicates. It SHALL NOT guarantee the absence of duplicates when a crash occurs after the producer transaction is committed and before the source offset is committed. Such residual duplicates MUST be absorbed by downstream idempotency (dedup key, business idempotency) or by a future L3 mechanism that commits the source offset inside the producer transaction. This limitation SHALL be documented. L3 (Kafka `send_offsets_to_transaction`) is deliberately deferred: the source offsets and the input's consumer-group metadata live in the input component, and handing them to the output's producer transaction requires an ack-chain protocol extension across components — a standalone change with live-cluster verification. SQL outputs cover the aggregated-batch window transactionally instead (see `sql-output`).
