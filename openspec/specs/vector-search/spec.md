...
Define the configuration, per-row retrieval and bounded-concurrency semantics, result JSON column shape, error semantics, and authentication of the `vector_search` processor: in-stream top-k similarity search against a Qdrant collection, completing the in-engine RAG loop alongside the embedding processor and the LLM processor. (Purpose derived from the `add-vector-search` change; refine as the capability evolves.)
