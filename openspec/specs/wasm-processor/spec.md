# Capability: WASM Processor

## Purpose

在沙箱中执行用户提供的 wasm 模块，对流的每一行做逐行变换：宿主将 batch 序列化为 NDJSON 行，逐行把 JSON 字节传入 guest 导出函数并回收输出，重组为新的 batch。沙箱通过 fuel 计量与线性内存限制防止 guest 无限循环或内存耗尽。（源自 `add-wasm-processor` 变更。）
