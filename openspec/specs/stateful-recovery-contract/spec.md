# stateful-recovery-contract Specification

## Purpose

Created by syncing the `enforce-stateful-recovery` change. Defines the durability contract for stateful Jobs: explicit durable/ephemeral state declarations, the distinction between the initial empty start and recovery-required starts, restore-before-consume recovery, and stable isolated state paths.
