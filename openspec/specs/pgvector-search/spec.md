# Capability: pgvector Search

## Purpose

Define the configuration, nearest-neighbor SQL generation (distance operator mapping), result JSON column shape, and error semantics of the `pgvector_search` processor: in-stream top-k similarity search against PostgreSQL tables with the pgvector extension, symmetric with the `pgvector` output table shape. (Purpose derived from the `add-pgvector-search` change; refine as the capability evolves.)
