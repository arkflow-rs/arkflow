...
Define the configuration, per-row request and bounded-concurrency semantics, message construction, error semantics, and authentication of the `llm` processor: in-stream LLM completions and rewrites through an OpenAI-compatible chat completions API. (Purpose derived from the `add-llm-processor` change; refine as the capability evolves.)
