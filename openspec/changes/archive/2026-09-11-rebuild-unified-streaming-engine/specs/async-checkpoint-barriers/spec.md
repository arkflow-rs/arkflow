...
### Requirement: Global checkpoint gate

The engine SHALL NOT pause the entire Job behind a global read/write gate to
take a checkpoint (legacy `checkpoint_gate` behavior).

*Rationale: replaced by asynchronous barrier flow; see barrier flows with data.*
