...
### Requirement: Dedicated Stream executor

The engine SHALL NOT execute StreamConfigs through a separate linear-pipeline
executor (`stream/mod.rs` Stream::run); StreamConfigs execute only as compiled
Jobs on the unified kernel.

*Rationale: replaced by stream-config compilation; see deterministic compilation.*
