...
impl S3Output {
    ...
    fn create_storage_client(config: &S3OutputConfig) -> Result<Arc<dyn ObjectStore>, Error> {
        match config.provider.to_lowercase().as_str() {
            ...
            other => Err(Error::Config(format!("Unsupported storage provider: {}", other))),
        }
    }
        ...
        match &config.credentials {
            ProviderCredentials::Aws(aws_creds) => {
                ...
                Ok(Arc::new(builder.build()
                    .map_err(|e| Error::Config(format!("Failed to build AWS S3 client: {}", e)))?))
            }
            _ => Err(Error::Config("Provider is 'aws', but AWS credentials are not provided or are of the wrong type.".to_string())),
        }
    }
    fn build_gcp_client(config: &S3OutputConfig) -> Result<Arc<dyn ObjectStore>, Error> {
        match &config.credentials {
            ProviderCredentials::Gcp(gcp_creds) => {
                ...
                Ok(Arc::new(builder.build()
                    .map_err(|e| Error::Config(format!("Failed to build GCP GCS client: {}", e)))?))
            }
            _ => Err(Error::Config("Provider is 'gcp', but GCP credentials are not provided or are of the wrong type.".to_string())),
        }
    }
...
impl Output for S3Output {
    ...
    async fn write(&self, msg: MessageBatch) -> Result<(), Error> {
        ...
        let (object_key, data) =tokio::task::spawn_blocking(move || -> Result<(Path, Vec<u8>), Error> {
            ...
            let filename_prefix_part = config_clone.prefix.as_ref().map_or_else(
                || "".to_string(),
                |p| format!("{}_", p.trim_matches('/')) 
            );
        ...
        s3_client_clone.put(&object_key, data.into())
            .await
...
impl S3Output {
    ...
    fn batch_to_parquet(msg: &MessageBatch) -> Result<Vec<u8>, Error> {
        ...
        writer.write(&batch)
            .map_err(|e| Error::Process(format!("Failed to write RecordBatch to Parquet: {}", e)))?;
