...
mod tests {
    ...
    use crate::processor::protobuf::{ProtobufProcessor, ProtobufProcessorConfig, ToType};

    #[tokio::test]
    async fn test_empty_batch() {
        let config = ProtobufProcessorConfig {
            proto_inputs: vec!["../examples".to_string()],
            proto_includes: None,
            message_type: "example.Person".to_string(),
            mode: ToType::ArrowToProtobuf,
            fields_to_include: None,
        };

        let processor = ProtobufProcessor::new(config).unwrap();
        let empty_batch = MessageBatch::new_empty();
        let result = processor.process(empty_batch).await.unwrap();
        assert!(result.is_empty());
    }

    #[tokio::test]
    async fn test_protobuf_to_arrow() {
        let config = ProtobufProcessorConfig {
            proto_inputs: vec!["../examples".to_string()],
            proto_includes: None,
            message_type: "example.Person".to_string(),
            mode: ToType::ProtobufToArrow(Default::default()),
            fields_to_include: None,
        };
        ...
        // Create test data
        let mut test_msg = DynamicMessage::new(processor.descriptor.clone());
        test_msg.set_field_by_name("name", "John Doe".to_string().into());
        test_msg.set_field_by_name("age", 30i32.into());
        test_msg.set_field_by_name("active", true.into());

        let mut buf = Vec::new();
        test_msg.encode(&mut buf).unwrap();

        let batch = MessageBatch::new_binary(vec![buf]);
        let result = processor.process(batch).await.unwrap();
        assert_eq!(result.len(), 1);

        let arrow_batch = result[0].clone();
        assert_eq!(arrow_batch.num_columns(), 3);
        assert_eq!(arrow_batch.num_rows(), 1);

        // Verify the data types and values
        let name_array = arrow_batch
            .column(0)
            .as_any()
            .downcast_ref::<StringArray>()
            .unwrap();
        assert_eq!(name_array.value(0), "John Doe");

        let age_array = arrow_batch
            .column(1)
            .as_any()
    ...
    #[tokio::test]
    async fn test_arrow_to_protobuf() {
        let config = ProtobufProcessorConfig {
            proto_inputs: vec!["../examples".to_string()],
            proto_includes: None,
            message_type: "example.Person".to_string(),
        ...
        let arrow_batch = MessageBatch::new_arrow(batch);
        let result = processor.process(arrow_batch).await.unwrap();
        assert_eq!(result.len(), 1);

        // Verify the Protobuf message
        let proto_batch = result[0].clone();
        let proto_data = proto_batch.to_binary("value").unwrap();
        assert_eq!(proto_data.len(), 1);

        let decoded_msg = DynamicMessage::decode(processor.descriptor.clone(), &proto_data[0]).unwrap();
        assert_eq!(
            decoded_msg.get_field_by_name("name").unwrap().as_str().unwrap(),
            "John Doe"
        );
        assert_eq!(
            decoded_msg.get_field_by_name("age").unwrap().as_i32().unwrap(),
            30
        );
        assert_eq!(
            decoded_msg
                .get_field_by_name("active")
                .unwrap()
                .as_bool()
                .unwrap(),
            true
    ...
    async fn test_field_filtering() {
        ...
        fields_to_include.insert("name".to_string());
        fields_to_include.insert("age".to_string());
        ...
        let config = ProtobufProcessorConfig {
            ...
            message_type: "example.Person".to_string(),
            mode: ToType::ArrowToProtobuf,
            fields_to_include: Some(fields_to_include),
        };

        let processor = ProtobufProcessor::new(config).unwrap();

        // Create test Arrow batch with all fields
        let schema = Arc::new(Schema::new(vec![
            Field::new("name", DataType::Utf8, false),
            Field::new("age", DataType::Int32, false),
            Field::new("active", DataType::Boolean, false),
        ...
        assert!(decoded_msg.get_field_by_name("name").is_some());
        assert!(decoded_msg.get_field_by_name("age").is_some());
        assert!(decoded_msg.get_field_by_name("active").is_none());
    }

    #[tokio::test]
    async fn test_invalid_config() {
        let config = ProtobufProcessorConfig {
            proto_inputs: vec!["non_existent_path".to_string()],
            proto_includes: None,
        ...
        let result = ProtobufProcessor::new(config);
