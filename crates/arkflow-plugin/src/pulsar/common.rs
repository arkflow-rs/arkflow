...
impl PulsarConfigValidator {
    ...
    pub fn validate_batching_config(
        config: &crate::input::pulsar::PulsarInputBatchingConfig,
    ) -> Result<(), Error> {
        if let Some(max_messages) = config.max_messages {
            ...
            if max_messages > 1000 {
                return Err(Error::Config("Max messages cannot exceed 1000".to_string()));
            }
        }
        ...
        if let Some(max_wait_ms) = config.max_wait_ms {
            if max_wait_ms == 0 {
                return Err(Error::Config(
                    "Max wait time must be greater than 0".to_string(),
                ));
            }
            if max_wait_ms > 60000 {
                return Err(Error::Config(
                    "Max wait time cannot exceed 60000ms (1 minute)".to_string(),
                ));
            }
        }
        ...
        if let Some(max_size_bytes) = config.max_size_bytes {
            if max_size_bytes == 0 {
                return Err(Error::Config(
                    "Max size bytes must be at least 1".to_string(),
                ));
            }
            if max_size_bytes > 10 * 1024 * 1024 {
                return Err(Error::Config(
                    "Max size bytes cannot exceed 10MB".to_string(),
                ));
            }
        }
    ...
    pub fn validate_producer_pool_config(
        ...
        if let Some(pool_size) = config.pool_size {
            ...
            if pool_size > 50 {
                return Err(Error::Config("Pool size cannot exceed 50".to_string()));
            }
        }

        if let Some(max_pending) = config.max_pending_messages {
            if max_pending == 0 {
                return Err(Error::Config(
                    "Max pending messages must be at least 1".to_string(),
                ));
            }
            if max_pending > 10000 {
                return Err(Error::Config(
                    "Max pending messages cannot exceed 10000".to_string(),
                ));
            }
        }
